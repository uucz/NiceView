...
第二阶段：围绕真实用户体验补齐 iOS 保存、筛选发现与收藏。反馈/举报和分享暂不做，PR 等 fork 开发完成后再评估。
...
- [x] 发布 v0.2.2，修复 iOS 保存图片走系统相册权限与 Photos 写入链路（2026-05-14，待实机保存回归）
- [ ] 抽象 API 查询模型并补充元数据/标签/分类接口
- [ ] 增加标签发现、标签预览与方向筛选
- [ ] 增加收藏
...
- iOS 保存图片后能在系统 Photos 中看到（v0.2.2 已发布，等待用户实机确认）
- 用户不手输标签也能发现可用标签
- 收藏图片不会被 30 张历史上限淘汰
